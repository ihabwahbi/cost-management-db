...
SOURCE_PATTERN = "OperationDashboard_Export_*.xlsx"

# Columns to keep from Operations export
SOURCE_COLUMNS = [
    "Project Number",
    "Operation Number",
    "Operation Name",
    ...
    "SAP WBS # / SO #",
    "Ops District",
    "Sub Business Line(s)",  # May contain multiple comma-separated codes
]


def find_input_file() -> Optional[Path]:
    """Find the Operations dashboard export file."""
    pattern = "OperationDashboard_Export_*.xlsx"
...
def standardize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rename_map = {
        ...
        "Operation Name": "wbs_name",
        "Customer": "client_name",
        "Rig": "rig",
        "SAP WBS # / SO #": "sap_wbs_raw",
        "Ops District": "ops_district",
        "Sub Business Line(s)": "sub_business_lines_raw",  # May contain comma-separated codes
    }
...
def select_output_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    output_cols = [
        ...
        "ops_district",
        "location",
        "sub_business_lines_raw",  # Comma-separated SBL codes to be parsed in Stage 2
    ]
...
def main(force: bool = False) -> bool:
    print("=" * 60)
    print("Stage 1: Extract WBS Data from Operations Report")
    print("=" * 60)
    ...
    if not force and cache.is_valid():
        ...
        print("\n" + "=" * 60)
        print("Stage 1 Complete: WBS from Operations (cached)")
        print("=" * 60)
        return True
    ...
    print("\n" + "=" * 60)
    print("Stage 1 Complete: WBS from Operations extracted")
    print("=" * 60)
    return True
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Extract WBS data from Operations dashboard export"
    )
    parser.add_argument(
        "--force", "-f",
        action="store_true",
